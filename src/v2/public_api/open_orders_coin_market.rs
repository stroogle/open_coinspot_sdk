...
impl CoinSpotPublic {
    ...
    pub async fn open_orders_coin_market(coin_symbol: &str, market: &str) -> CoinSpotResult<OpenOrders>{
        let url = format!("https://www.coinspot.com.au/pubapi/v2/orders/open/{}/{}", coin_symbol, market);
...
mod tests {
    ...
    #[tokio::test]
    async fn test_open_orders_coin_market() {
    
        let result: CoinSpotResponse<OpenOrders>;
        result = CoinSpotPublic::open_orders_coin("btc")
        .await
        .unwrap();
    ...
    #[tokio::test]
    async fn test_open_orders_fake_coin_market() {
        let result2 = CoinSpotPublic::open_orders_coin_market("sdfsdf", "usdt")
        .await
        .unwrap();
